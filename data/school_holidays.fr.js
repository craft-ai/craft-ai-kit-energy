...
function formatHolidays(data) {
  return [].concat(...Object.values(data)).reduce(reduceHolidays, {});
}

function indexHolidays(years, current) {
  const start = parseDate(current.debut).plus(oneDay);
  const year = start.month <= 7 ? start.year - 1 : start.year;
  const value = { start, end: parseDate(current.fin) };

  if (year in years) years[year].push(value);
  else years[year] = [value];
...
function parse(text) {
  ...
  const exceptions = toArray(data.exceptionszones.exceptionzone).reduce((exceptions, exception) => {
    const academyId = exception.academie;
    const year = exception.annee;
    const holidays = zones[exception.zone][year];

    /* istanbul ignore if */
    if (academyId in exceptions) exceptions[academyId][year] = holidays;
    else exceptions[academyId] = { [year]: holidays };
  ...
  const regions = data.academies.academie.reduce((regions, academy) => {
    ...
    toArray(academy.departement).forEach((region) => regions[region.numero] = holidays);
...
function patch(data) {
  ...
  function addRegionTo(academies, academyId, regionId) {
    ...
    else academy.departement = regions.concat([{ numero: regionId }]);
  }
  ...
  function findAcademy(academies, academyId) {
    ...
    /* istanbul ignore if */
    if (!academy) throw new Error(`Unable to patch ${academyId} academy.`);
  ...
  function removeRegionFrom(academies, academyId, regionId) {
    ...
    else academy.departement = filtered;
  }
}

function reduceHolidays(years, holidays) {
  const start = holidays.start;
  const days = Math.floor(holidays.end.diff(start).as('days'));
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (year in years) {
      ...
      if (month in months) months[month][day] = true;
      else months[month] = { [day]: true };
    } else years[year] = { [month]: { [day]: true } };
  }
