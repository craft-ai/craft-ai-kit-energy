...
test('fails computing rolling evaluations with invalid parameters', (t) => {
  ...
  const interval = { from: TRAINING_RECORDS[0][DATE], to: TRAINING_RECORDS[length - 1][DATE] };

  const INVALID_PERIODS = [null, { days: '25' }, { minutes: 'one' }, '1DAY', '1DT11H45', 0, [], () => 5];

  return Promise.all([]
    .concat(INVALID_OBJECTS.map((states) => t.throwsAsync(endpoint.evaluate(states))))
    .concat(INVALID_DATES.map((date) => t.throwsAsync(endpoint.evaluate({ from: date }))))
    .concat(INVALID_DATES.map((date) => t.throwsAsync(endpoint.evaluate({ to: date }))))
    .concat(INVALID_OBJECTS.map((option) => t.throwsAsync(endpoint.evaluate(interval, option))))
    .concat(INVALID_PERIODS.map((period) => t.throwsAsync(endpoint.evaluate(interval, { period }))))
  );
});
...
test('computes rolling evaluation', (t) => {
  ...
  return Promise
    .all(PERIODS.map((period) => endpoint.evaluate(interval, { period })))
    .then((results) => {
    ...
    .then(() => Promise
      .all([endpoint.evaluate(interval, {}), endpoint.evaluate(interval)]))
    .then((results) => {
      const result = results[0];
      result.forEach((report) => t.true(Is.report(report)));
...
function getPredictions(reports) {
  return reports.reduce((predictions, report) => predictions.concat(report.values), []);
}
